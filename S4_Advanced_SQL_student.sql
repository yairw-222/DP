...
WHERE gbdatum<'1980-1-1' AND functie = 'VERKOPER' OR functie = 'TRAINER';
...
SELECT cursus,begindatum, COUNT(*) AS aantal_cursisten
FROM inschrijvingen
...
--   JAV    | 4
--   OAG    | 2
SELECT cursus , COUNT(cursus) AS aantal_keren
FROM uitvoeringen
GROUP BY cursus
...
SELECT (MAX(gbdatum)-MIN(gbdatum))/365,AVG(NOW()::date-gbdatum)/365 FROM medewerkers
...
SELECT  AS aantal_medewerkers,AVG(comm) AS commissie_medewerkers, AVG(comm) AS comissie_verkoper
FROM medewerkers
GROUP BY mnr,comm
